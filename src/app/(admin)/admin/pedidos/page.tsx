...
async function OrdersTable({ status, search, page }: { status?: string; search?: string; page: number }) {
  const { orders, total, pages } = await getOrders(status, search, page)
    ...
    <div>
      ...
      {pages > 1 && (
        <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
          <p className="text-xs text-gray-500">{total} pedidos · página {page} de {pages}</p>
          <div className="flex gap-1">
            {page > 1 && (
              <Link href={buildUrl(page - 1)}>
                <Button size="sm" variant="ghost"><ChevronLeft className="w-4 h-4" /></Button>
              </Link>
            )}
            {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
              ...
              return (
                <Link key={p} href={buildUrl(p)}>
                  <Button size="sm" variant={p === page ? 'default' : 'ghost'} className={p === page ? 'bg-orange-500 text-white hover:bg-orange-600' : ''}>
                    {p}
                  </Button>
                </Link>
              )
...
export default async function PedidosAdminPage({ searchParams }: { searchParams: Promise<{ status?: string; q?: string; page?: string }> }) {
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <Suspense fallback={<div className="p-8 text-center text-gray-500">A carregar...</div>}>
            <OrdersTable status={status} search={q} page={page} />
          </Suspense>
        </div>
      </div>
    </div>
